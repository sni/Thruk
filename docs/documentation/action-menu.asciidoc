    ...
    "menu":[
      {
        ...
        "html":"<b>test</b><input type='text'><button onclick='test_form_handler(this)'>ok</button>"
      }
    ]
  });
}
...
function test_form_handler(btn) {
  // prevent clicking twice and give user some response by adding a loading gif to the button
  jQuery(btn).attr('disabled', true).html("<img src='"+url_prefix+"themes/"+theme+"/images/loading-icon.gif'>");

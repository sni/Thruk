...
=== rest_api_enabled
{% include new_since.ad version="2.24" %}
The link:rest.html[rest api] is enabled by default, but you can disable or restrict it.

Valid options are:
  - 0: rest api disabled
  - 1: rest api enabled for everyone
  - 2: rest api enabled for admins only
  - 3: rest api enabled from command line only
...
ex.:
  ...
  <Component Thruk::Plugin::ConfigTool>
    ...
    # default_keys_host       = host_name, use, alias, address, contact_groups
...
specific readonly patterns from the Component Backend peer configtool. Read
details on the link:/documentation/configuration.html#peer[configtool section].

=== default_keys_host

Sets the default attributes when creating a new host. Can contain custom variables
like `_VAR`.

ex.:

    default_keys_host       = host_name, use, alias, address, contact_groups, _WORKER

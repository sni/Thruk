...
sub index {
    ...
    my $peer_columns = ['peer_name', 'peer_addr'];
    ...
    if($type eq 'timeperiods') {
        $c->{'db'}->get_timeperiods(sort => 'name', remove_duplicates => 1, pager => 1, extra_columns => $peer_columns);
        $c->stash->{template} = 'config_timeperiods.tt';
    }

    # commands
    if($type eq 'commands') {
        $c->{'db'}->get_commands(sort => 'name', remove_duplicates => 1, pager => 1, extra_columns => $peer_columns);
        $c->stash->{template} = 'config_commands.tt';
    }

    # contacts
    elsif($type eq 'contacts') {
        $c->{'db'}->get_contacts(sort => 'name', remove_duplicates => 1, pager => 1, extra_columns => $peer_columns);
        $c->stash->{template} = 'config_contacts.tt';
    }

    # contactgroups
    elsif($type eq 'contactgroups') {
        $c->{'db'}->get_contactgroups(sort => 'name', remove_duplicates => 1, pager => 1, extra_columns => $peer_columns);
    ...
    # hosts
        ...
        $c->{'db'}->get_hosts(sort => 'name', remove_duplicates => 1, pager => 1, extra_columns => ['contacts', 'contact_groups', @{$peer_columns}], filter => $filter );
    ...
    # services
        ...
        $c->{'db'}->get_services(sort => [ 'host_name', 'description' ], remove_duplicates => 1, pager => 1, extra_columns => ['contacts', 'contact_groups', @{$peer_columns}], filter => $filter);
        $c->stash->{template} = 'config_services.tt';
    }

    # hostgroups
    elsif($type eq 'hostgroups') {
        $c->{'db'}->get_hostgroups(sort => 'name', remove_duplicates => 1, pager => 1, extra_columns => $peer_columns);
        $c->stash->{template} = 'config_hostgroups.tt';
    }

    # servicegroups
    elsif($type eq 'servicegroups') {
        $c->{'db'}->get_servicegroups(sort => 'name', remove_duplicates => 1, pager => 1, extra_columns => $peer_columns);
